...
void initializeApplication(void)
{
	...
	grid_mtx = (pthread_mutex_t**) malloc(sizeof(pthread_mutex_t*) * numRows);
	for (int i=0; i<numRows; i++)
	{
		grid_mtx[i] = (pthread_mutex_t*) malloc(sizeof(pthread_mutex_t) * numCols);
		for (int j=0; j<numCols; j++)
			pthread_mutex_init(&grid_mtx[i][j], NULL);
	}
    ...
    for (int k = 0; k<numBoxes; k++)
    {
		...
		pthread_mutex_lock(&grid_mtx[robotLoc[k][0]][robotLoc[k][1]]);
		pthread_mutex_lock(&grid_mtx[boxLoc[k][0]][boxLoc[k][1]]);
...
{
	int id = robot->id;
	int rx = robotLoc[id][0];
	int ry = robotLoc[id][1];
	int bx = boxLoc[id][0];
	int by = boxLoc[id][1];
	...
	{
		case NORTH:
			pthread_mutex_lock(&grid_mtx[bx][by+1]);
			pthread_mutex_unlock(&grid_mtx[bx][by]);
			pthread_mutex_lock(&grid_mtx[rx][ry+1]);
			pthread_mutex_unlock(&grid_mtx[rx][ry]);
			robotLoc[robot->id][1]++;
			boxLoc[robot->id][1]++;
			robot->dy_bd--;
			break;
		case WEST:
			pthread_mutex_lock(&grid_mtx[bx-1][by]);
			pthread_mutex_unlock(&grid_mtx[bx][by]);
			pthread_mutex_lock(&grid_mtx[rx-1][ry]);
			pthread_mutex_unlock(&grid_mtx[rx][ry]);
			robotLoc[robot->id][0]--;
			boxLoc[robot->id][0]--;
			robot->dx_bd++;
...
void end(Robot* robot)
{
	...
	pthread_mutex_unlock(&grid_mtx[boxLoc[id][0]][boxLoc[id][1]]);
	pthread_mutex_unlock(&grid_mtx[robotLoc[id][0]][robotLoc[id][1]]);
